...
// TAWSE - custom function to get field defs for all modules
// Taken from /modules/EmailTemplates/EmailTemplate.php EmailTemplate::generateFieldDefsJS()
...
$xtpl->assign("FIELD_DEFS_JS", generate_field_defs());
...
if(true) {
	...
	if($has_campaign) {
        ...
        // TAWSE add additional modules
        // Use parent_type_display for master module list
        $module_list = $app_list_strings['parent_type_display'];
        $ignore_modules = array('Accounts', 'Contacts', 'Leads', 'Prospects', 'Users');
        foreach($module_list as $module => $label) {
            if (!in_array($module, $ignore_modules)) {
                $dropdown.= '<option value="'.$module.'">'.$app_list_strings['moduleListSingular'][$module].'</option>';
            }
        }
        // TAWSE
